...
class DB extends DbControl{
    ...
    public function selectCountQuestion($table = NULL, $columns  = NULL, $where = NULL,  $orderby  = NULL, $groupby  = NULL){
		...
		{
		//$sql = 'SELECT '. $col .' FROM `'. $table .'` '.$add;
		$sql = 'SELECT COUNT(`questions`.`id`) AS id FROM `instructions` LEFT JOIN `questions` ON `instructions`.`id` = `questions`.`instructionID` WHERE `instructions`.`topicID` = '.$where.' ';	
				$rows = array();
				$dbh = $this->construct();	
				$sth = $dbh->query($sql);
	...
	public function selectINS($table = NULL, $columns  = NULL, $where  = NULL,  $orderby  = NULL, $groupby  = NULL)
	{
		$add = is_array($where)  && count($where) > 0 ? ' WHERE '.$keys[0].' IN ('.$where[$keys[0]].') ' : '';
		$add .= parent::orderByClause($orderby);
   ...
   $sql = 'SELECT '. $col .' FROM `'. $table .'` '.$add;
	...
	public function loadFilez($from, $to){
		
			//unset($to
			
			$f = $_FILES[$from]['tmp_name'];
			if($isMove = move_uploaded_file($from, $to))
			{
				echo 'Done';
			}else{
				echo 'Pass';
	...
	public function createTableSemesters($id){
			...
			//students levels
				$sc = array(
				array('name'=>'id', 'PK'=>true, 'type'=>'INT', 'num'=>6, 'FK'=>false, 'ref'=>'', 'refname'=>''),
				array('name'=>'studentID', 'PK'=>false, 'type'=>'INT', 'num'=>6, 'FK'=>true, 'ref'=>'exam_students', 'refname'=>'id'),
...
$_SESSION['a']["fullname"] = ucfirst(strtolower($d->surname)).", ".ucfirst(strtolower($d->middlename))." ".ucfirst(strtolower($d->firstname));  
	...
	$msg = '<i class="label label-danger">You already have an active session or You did not properly Logout of the previous session. Kindly see the administrator</i>';
	...
	else
	{
		...
		$this->insert('exam_login', $log_details);
...
$_SESSION['a']["fullname"] = ucfirst(strtolower($d->surname)).", ".ucfirst(strtolower($d->middlename))." ".ucfirst(strtolower($d->firstname));  
	...
	$d = $this->selectOne('exam_staff', NULL, array('empno'=>$username));
	if (count($d) == 1){
		//print_r($d);
	if ($d->password = $password){
				session_start();
